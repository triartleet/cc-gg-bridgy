...
// startedAt) moments later, and bridgy's 2s poll pairs the two. Pairing is
...
// birth, opened before bridgy activated) just keep the conservative
...
export function initTabTracker(context: vscode.ExtensionContext): void {
  for (const group of vscode.window.tabGroups.all)
    for (const tab of group.tabs) if (isClaudePanel(tab) && tab.isActive) lastActive = tab
...
export function drainTabBindings(ws: string): void {
  const now = Date.now()
  while (pending.length > 0 && now - pending[0].at > PAIR_WINDOW_MS) pending.shift()
...
export function boundClaudeTabs(): { tab: vscode.Tab; sessionId: string }[] {
  return [...bindings.entries()].map(([tab, sessionId]) => ({ tab, sessionId }))
}
